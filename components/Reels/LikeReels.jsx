...
const LikeReels = ({ reelId, initialLikeCount = 0 }) => {
  ...
  const likeReel = async () => {
    ...
    try {
      ...
      const q = query(likesCollection, where('reelId', '==', reelId), where('userId', '==', user.id));
      const querySnapshot = await getDocs(q);

      if (querySnapshot.empty) {
        // User has not liked the reel, add a like
        await addDoc(likesCollection, { reelId, userId: user.id, timestamp: new Date() });
  ...
  return (
    <TouchableOpacity style={styles.likeButton} onPress={likeReel} disabled={isLoading || !user}>
